...
object Huffman {
  ...
  // Part 1: Basics
    def weight(tree: CodeTree): Int =  tree match {
      case Fork(left, right,char,w) => weight(left) + weight(right)
      case Leaf(char, w) => w
    }
  ...
  def until(validation: List[CodeTree] => Boolean, reduction: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] =
    if(validation(trees)) trees
    else until(validation, reduction)(reduction(trees))
  /**
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def traverse( remaingTree:CodeTree,bits:List[Bit] ):List[Char] = remaingTree match {
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] =
      ...
      table.filter((code) => char == code._1).head._2
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      def prepend (b:Bit)(c:Code) :Code = (c._1 , b::c._2)
